...
import jp.co.dmm.customize.endpoint.mg.mg0160.excel.MgExcelReaderTax;
import jp.co.dmm.customize.endpoint.mg.mg0160.excel.MgExcelSheetTax;
import jp.co.dmm.customize.endpoint.mg.mg0160.excel.MgUploadSOTax;
import jp.co.dmm.customize.endpoint.mg.mg0160.excel.MgUploadValidatorTax;
 ...
 */
@ApplicationScoped
public class Mg0160Service extends BasePagingService  {


	@Inject private Mg0160Repository repository;
	@Inject private MgUploadValidatorTax validator;
	@Inject private MgExcelReaderTax reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookTax> bookMap = new HashMap<String, MgExcelBookTax>();
	...
	public Mg0160SearchResponse init(Mg0160SearchRequest req) {
		final Mg0160SearchResponse res = createResponse(Mg0160SearchResponse.class, req);
	...
	public Mg0160SearchResponse search(Mg0160SearchRequest req) {
		...
		final Mg0160SearchResponse res = createResponse(Mg0160SearchResponse.class, req, allCount);
	...
	public Mg0160RemoveResponse delete(Mg0160RemoveRequest req) {
		final Mg0160RemoveResponse res = createResponse(Mg0160RemoveResponse.class, req);
	...
	public Response download(Mg0160SearchRequest req) {
		...
		final String fileName = MessageFormat.format("消費税マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookTax book = new MgExcelBookTax();

		book.sheet = new MgExcelSheetTax();
		book.sheet.entityList = repository.getMasterData((Mg0160SearchRequest) req);

		final MgUploadSOTax streaming = new MgUploadSOTax(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOTax streaming = new MgUploadSOTax(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookTax book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookTax book) throws IOException {
