...
import jp.co.dmm.customize.endpoint.mg.mg0190.excel.MgExcelReaderBumon;
import jp.co.dmm.customize.endpoint.mg.mg0190.excel.MgExcelSheetBumon;
import jp.co.dmm.customize.endpoint.mg.mg0190.excel.MgUploadSOBumon;
import jp.co.dmm.customize.endpoint.mg.mg0190.excel.MgUploadValidatorBumon;
...
@ApplicationScoped
public class Mg0190Service extends BasePagingService  {


	@Inject private Mg0190Repository repository;
	@Inject private MgUploadValidatorBumon validator;
	@Inject private MgExcelReaderBumon reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookBumon> bookMap = new HashMap<String, MgExcelBookBumon>();
	...
	public Mg0190SearchResponse init(Mg0190SearchRequest req) {
		final Mg0190SearchResponse res = createResponse(Mg0190SearchResponse.class, req);
	...
	public Mg0190SearchResponse search(Mg0190SearchRequest req) {
		...
		final Mg0190SearchResponse res = createResponse(Mg0190SearchResponse.class, req, allCount);
	...
	public Mg0190Response delete(Mg0190Request req) {
		final Mg0190Response res = createResponse(Mg0190Response.class, req);
	...
	public Response download(Mg0190SearchRequest req) {
		...
		final String fileName = MessageFormat.format("部門マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookBumon book = new MgExcelBookBumon();

		book.sheet = new MgExcelSheetBumon();
		book.sheet.entityList = repository.getMasterData((Mg0190SearchRequest) req);

		final MgUploadSOBumon streaming = new MgUploadSOBumon(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOBumon streaming = new MgUploadSOBumon(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookBumon book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookBumon book) throws IOException {
