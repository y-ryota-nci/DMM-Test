...
import jp.co.dmm.customize.endpoint.mg.mg0170.excel.MgExcelReaderMny;
import jp.co.dmm.customize.endpoint.mg.mg0170.excel.MgExcelSheetMny;
import jp.co.dmm.customize.endpoint.mg.mg0170.excel.MgUploadSOMny;
import jp.co.dmm.customize.endpoint.mg.mg0170.excel.MgUploadValidatorMny;
 ...
 */
@ApplicationScoped
public class Mg0170Service extends BasePagingService  {


	@Inject private Mg0170Repository repository;
	@Inject private MgUploadValidatorMny validator;
	@Inject private MgExcelReaderMny reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookMny> bookMap = new HashMap<String, MgExcelBookMny>();
	...
	public Mg0170SearchResponse init(Mg0170SearchRequest req) {
		final Mg0170SearchResponse res = createResponse(Mg0170SearchResponse.class, req);
	...
	public Mg0170SearchResponse search(Mg0170SearchRequest req) {
		...
		final Mg0170SearchResponse res = createResponse(Mg0170SearchResponse.class, req, allCount);
	...
	public Mg0170RemoveResponse delete(Mg0170RemoveRequest req) {
		final Mg0170RemoveResponse res = createResponse(Mg0170RemoveResponse.class, req);
	...
	public Response download(Mg0170SearchRequest req) {
		...
		final String fileName = MessageFormat.format("通貨マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookMny book = new MgExcelBookMny();

		book.sheet = new MgExcelSheetMny();
		book.sheet.entityList = repository.getMasterData((Mg0170SearchRequest) req);

		final MgUploadSOMny streaming = new MgUploadSOMny(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOMny streaming = new MgUploadSOMny(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookMny book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookMny book) throws IOException {
