...
import jp.co.dmm.customize.endpoint.mg.mg0090.excel.MgExcelReaderBnkacc;
import jp.co.dmm.customize.endpoint.mg.mg0090.excel.MgExcelSheetBnkacc;
import jp.co.dmm.customize.endpoint.mg.mg0090.excel.MgUploadSOBnkacc;
import jp.co.dmm.customize.endpoint.mg.mg0090.excel.MgUploadValidatorBnkacc;
...
@ApplicationScoped
public class Mg0090Service extends MgUploadService  {


	@Inject private Mg0090Repository repository;
	@Inject private MgUploadValidatorBnkacc validator;
	@Inject private MgExcelReaderBnkacc reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookBnkacc> bookMap = new HashMap<String, MgExcelBookBnkacc>();
	 ...
	 * @return
	 */
	public Mg0090SearchResponse init(Mg0090SearchRequest req) {
		final Mg0090SearchResponse res = createResponse(Mg0090SearchResponse.class, req);
	 ...
	 * @return
	 */
	public Mg0090SearchResponse search(Mg0090SearchRequest req) {
		...
		final Mg0090SearchResponse res = createResponse(Mg0090SearchResponse.class, req, allCount);
	 ...
	 * @return
	 */
	public Mg0090Response delete(Mg0090Request req) {
		final Mg0090Response res = createResponse(Mg0090Response.class, req);
		...
		for (BodyPart bodyPart : multiPart.getBodyParts()) {
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					MgExcelBookBnkacc book = new MgExcelBookBnkacc();
					parse(bais, book);
					...
					if (validator.validate(book)) {
						...
						MgUploadSOBnkacc streaming = new MgUploadSOBnkacc(book);
	...
	public Response download(BasePagingRequest req) {
		...
		final String fileName = MessageFormat.format("銀行口座マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookBnkacc book = new MgExcelBookBnkacc();

		book.sheet = new MgExcelSheetBnkacc();
		book.sheet.entityList = repository.getMasterData((Mg0090SearchRequest) req);

		final MgUploadSOBnkacc streaming = new MgUploadSOBnkacc(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookBnkacc book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookBnkacc book) throws IOException {
