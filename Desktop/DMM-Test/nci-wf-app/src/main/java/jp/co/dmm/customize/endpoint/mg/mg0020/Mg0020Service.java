...
import jp.co.dmm.customize.endpoint.mg.mg0020.excel.MgExcelReaderItmexps;
import jp.co.dmm.customize.endpoint.mg.mg0020.excel.MgExcelSheetItmexps;
import jp.co.dmm.customize.endpoint.mg.mg0020.excel.MgUploadSOItmexps;
import jp.co.dmm.customize.endpoint.mg.mg0020.excel.MgUploadValidatorItmexps;
...
@ApplicationScoped
public class Mg0020Service extends MgUploadService  {

	@Inject private MgUploadValidatorItmexps validator;
	@Inject private MgExcelReaderItmexps reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookItmexps> bookMap = new HashMap<String, MgExcelBookItmexps>();
	...
	public Mg0020SearchResponse init(Mg0020SearchRequest req) {
		final Mg0020SearchResponse res = createResponse(Mg0020SearchResponse.class, req);
	...
	public Mg0020SearchResponse search(Mg0020SearchRequest req) {
		...
		final Mg0020SearchResponse res = createResponse(Mg0020SearchResponse.class, req, allCount);
	...
	public Mg0020Response delete(Mg0020Request req) {
		final Mg0020Response res = createResponse(Mg0020Response.class, req);
	...
	public Response download(BasePagingRequest req) {
		...
		final String fileName = MessageFormat.format("費目マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookItmexps book = new MgExcelBookItmexps();

		book.sheet = new MgExcelSheetItmexps();
		book.sheet.entityList = repository.getMasterData((Mg0020SearchRequest) req);

		final MgUploadSOItmexps streaming = new MgUploadSOItmexps(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOItmexps streaming = new MgUploadSOItmexps(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookItmexps book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookItmexps book) throws IOException {
