...
import jp.co.dmm.customize.endpoint.mg.mg0240.excel.MgExcelReaderPaySite;
import jp.co.dmm.customize.endpoint.mg.mg0240.excel.MgExcelSheetPaySite;
import jp.co.dmm.customize.endpoint.mg.mg0240.excel.MgUploadSOPaySite;
import jp.co.dmm.customize.endpoint.mg.mg0240.excel.MgUploadValidatorPaySite;
 ...
 */
@ApplicationScoped
public class Mg0240Service extends BasePagingService  {


	@Inject private Mg0240Repository repository;
	@Inject private MgUploadValidatorPaySite validator;
	@Inject private MgExcelReaderPaySite reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookPaySite> bookMap = new HashMap<String, MgExcelBookPaySite>();
	 ...
	 * @return
	 */
	public Mg0240SearchResponse init(Mg0240SearchRequest req) {
		final Mg0240SearchResponse res = createResponse(Mg0240SearchResponse.class, req);
		...
		// 支払サイト（月）の選択
		res.paySiteMOpts = repository.getSelectItems(u.getCorporationCode(), "paySiteMOpts", true);

		// 支払サイト（日）の選択
		res.paySiteNOpts = repository.getSelectItems(u.getCorporationCode(), "paySiteNOpts", true);
	 ...
	 * @return
	 */
	public Mg0240SearchResponse search(Mg0240SearchRequest req) {
		...
		final Mg0240SearchResponse res = createResponse(Mg0240SearchResponse.class, req, allCount);
	 ...
	 * @return
	 */
	public Mg0240RemoveResponse delete(Mg0240RemoveRequest req) {
		final Mg0240RemoveResponse res = createResponse(Mg0240RemoveResponse.class, req);
		int delTotalCnt = repository.delete(req);
		res.success = (delTotalCnt > 0);
		return res;
	}

	public Response download(Mg0240SearchRequest req) {
		final LoginInfo u = sessionHolder.getLoginInfo();
		final String corporationCode = u.getCorporationCode();
		final String fileName = MessageFormat.format("支払サイトマスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookPaySite book = new MgExcelBookPaySite();

		book.sheet = new MgExcelSheetPaySite();
		book.sheet.entityList = repository.getMasterData((Mg0240SearchRequest) req);

		final MgUploadSOPaySite streaming = new MgUploadSOPaySite(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOPaySite streaming = new MgUploadSOPaySite(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookPaySite book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookPaySite book) throws IOException {
