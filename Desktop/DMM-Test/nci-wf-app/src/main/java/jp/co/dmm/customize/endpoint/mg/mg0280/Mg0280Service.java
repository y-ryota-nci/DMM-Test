...
import jp.co.dmm.customize.endpoint.mg.mg0280.excel.MgExcelReaderMda;
import jp.co.dmm.customize.endpoint.mg.mg0280.excel.MgExcelSheetMda;
import jp.co.dmm.customize.endpoint.mg.mg0280.excel.MgUploadSOMda;
import jp.co.dmm.customize.endpoint.mg.mg0280.excel.MgUploadValidatorMda;
 ...
 */
@ApplicationScoped
public class Mg0280Service extends BasePagingService  {


	@Inject private Mg0280Repository repository;
	@Inject private MgUploadValidatorMda validator;
	@Inject private MgExcelReaderMda reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookMda> bookMap = new HashMap<String, MgExcelBookMda>();
	 ...
	 * @return
	 */
	public Mg0280SearchResponse init(Mg0280SearchRequest req) {
		final Mg0280SearchResponse res = createResponse(Mg0280SearchResponse.class, req);
	 ...
	 * @return
	 */
	public Mg0280SearchResponse search(Mg0280SearchRequest req) {
		...
		final Mg0280SearchResponse res = createResponse(Mg0280SearchResponse.class, req, allCount);
	 ...
	 * @return
	 */
	public Mg0280RemoveResponse delete(Mg0280RemoveRequest req) {
		final Mg0280RemoveResponse res = createResponse(Mg0280RemoveResponse.class, req);
		int delTotalCnt = repository.delete(req);
		res.success = (delTotalCnt > 0);
		return res;
	}

	public Response download(Mg0280SearchRequest req) {
		final LoginInfo u = sessionHolder.getLoginInfo();
		final String corporationCode = u.getCorporationCode();
		final String fileName = MessageFormat.format("メディアマスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookMda book = new MgExcelBookMda();

		book.sheet = new MgExcelSheetMda();
		book.sheet.entityList = repository.getMasterData((Mg0280SearchRequest) req);

		final MgUploadSOMda streaming = new MgUploadSOMda(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOMda streaming = new MgUploadSOMda(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookMda book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookMda book) throws IOException {
