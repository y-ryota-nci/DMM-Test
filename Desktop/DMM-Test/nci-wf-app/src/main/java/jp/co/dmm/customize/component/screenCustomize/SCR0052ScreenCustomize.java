...
import jp.co.dmm.customize.endpoint.sp.sp0011.Sp0011GetRequest;
...
@ScreenCustomizable
@Named
public class SCR0052ScreenCustomize extends DmmScreenCustomize {
	...
	public void beforeInitRender(Vd0310InitRequest req, Vd0310InitResponse res, RuntimeContext ctx) {
		...
		if (StringUtils.isNotEmpty(companyCd) && StringUtils.isNotEmpty(splrCd)) {
			// 取引先情報取得用リクエスト
	 		Sp0011GetRequest splrReq = new Sp0011GetRequest();
			splrReq.companyCd = companyCd;
			splrReq.splrCd = splrCd;
			...
			final Map<String, List<UserDataEntity>> tables = new HashMap<>();
			{
				...
				final String sql = sqlService.get("SP0020_05");
			...
			// 取引先口座明細情報をユーザデータとして読み込み
			{
				...
				final String sql = sqlService.get("SP0020_06");
				final List<UserDataEntity>  userDataList = loader.getUserData(tableName, sql, params);
				tables.put(tableName, userDataList);
			}
	...
	public void beforeValidate(Vd0310ExecuteRequest req, Vd0310ExecuteResponse res) {
		...
		for (PartsContainerRow row : grid.rows) {
			...
			// 変更取引先申請の場合、CFO室承認時のみ、振込先銀行口座コード（SuperStream）の重複チェックを行う
			if ("0000000003".equals(req.contents.activityDefCode)
