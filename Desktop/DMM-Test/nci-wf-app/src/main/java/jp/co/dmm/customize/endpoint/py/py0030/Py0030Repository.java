 ...
 */
@ApplicationScoped
public class Py0030Repository extends BaseRepository {
	 ...
	 * @return
	 */
	public int count(Py0030SearchRequest req) {
		StringBuilder sql = new StringBuilder(
				getSql("PY0030_01").replaceFirst(REPLACE, "count(*)"));
	 ...
	 * @return
	 */
	public List<?> select(Py0030SearchRequest req, Py0030SearchResponse res) {
		if (res.allCount == 0) {
			return new ArrayList<>();
		}
		StringBuilder sql = new StringBuilder(
				getSql("PY0030_01").replaceFirst(REPLACE, getSql("PY0030_02")));
		final List<Object> params = new ArrayList<>();

		fillCondition(req, sql, params, true);

		return select(Py0030Entity.class, sql, params.toArray());
	}

	private void fillCondition(Py0030SearchRequest req, StringBuilder sql, List<Object> params, boolean paging) {
...
//		}
	 ...
	 * @return
	 */
	public int countPay(Py0030ValidRequest req) {
		final String sql = getSql("PY0030_03");
	 ...
	 * @return
	 */
	public int getScreenProcessId(String companyCd, String payNo) {
		final PayInf p = getPayInf(companyCd, payNo);
		final String screenCode = PurordTp.EXPENSE.equals(p.getPurordTp()) ? "SCR0080" : "SCR0078";

		StringBuilder sql = new StringBuilder(getSql("PY0030_04"));
		final List<Object> params = new ArrayList<>();
	...
	private PayInf getPayInf(String companyCd, String payNo) {
		PayInfPK key = new PayInfPK();
		key.setCompanyCd(companyCd);
		key.setPayNo(payNo);
		return em.find(PayInf.class, key);
	}
