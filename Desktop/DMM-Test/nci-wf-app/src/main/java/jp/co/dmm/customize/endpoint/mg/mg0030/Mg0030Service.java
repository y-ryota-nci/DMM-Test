...
import jp.co.dmm.customize.endpoint.mg.mg0030.excel.MgExcelReaderItmexpsChrmst;
import jp.co.dmm.customize.endpoint.mg.mg0030.excel.MgExcelSheetItmexpsChrmst;
import jp.co.dmm.customize.endpoint.mg.mg0030.excel.MgUploadSOItmexpsChrmst;
import jp.co.dmm.customize.endpoint.mg.mg0030.excel.MgUploadValidatorItmexpsChrmst;
...
@ApplicationScoped
public class Mg0030Service extends BasePagingService  {


	@Inject private Mg0030Repository repository;
	@Inject private MgUploadValidatorItmexpsChrmst validator;
	@Inject private MgExcelReaderItmexpsChrmst reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookItmexpsChrmst> bookMap = new HashMap<String, MgExcelBookItmexpsChrmst>();
	...
	public Mg0030SearchResponse init(Mg0030SearchRequest req) {
		final Mg0030SearchResponse res = createResponse(Mg0030SearchResponse.class, req);
	...
	public Mg0030SearchResponse search(Mg0030SearchRequest req) {
		...
		final Mg0030SearchResponse res = createResponse(Mg0030SearchResponse.class, req, allCount);
	...
	public Mg0030RemoveResponse delete(Mg0030RemoveRequest req) {
		final Mg0030RemoveResponse res = createResponse(Mg0030RemoveResponse.class, req);
	...
	public Response download(Mg0030SearchRequest req) {
		...
		final String fileName = MessageFormat.format("費目関連マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookItmexpsChrmst book = new MgExcelBookItmexpsChrmst();

		book.sheet = new MgExcelSheetItmexpsChrmst();
		book.sheet.entityList = repository.getMasterData((Mg0030SearchRequest) req);

		final MgUploadSOItmexpsChrmst streaming = new MgUploadSOItmexpsChrmst(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOItmexpsChrmst streaming = new MgUploadSOItmexpsChrmst(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookItmexpsChrmst book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookItmexpsChrmst book) throws IOException {
