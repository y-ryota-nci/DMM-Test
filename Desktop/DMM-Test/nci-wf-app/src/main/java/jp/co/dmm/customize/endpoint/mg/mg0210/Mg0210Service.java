...
import jp.co.dmm.customize.endpoint.mg.mg0210.excel.MgExcelReaderHldtax;
import jp.co.dmm.customize.endpoint.mg.mg0210.excel.MgExcelSheetHldtax;
import jp.co.dmm.customize.endpoint.mg.mg0210.excel.MgUploadSOHldtax;
import jp.co.dmm.customize.endpoint.mg.mg0210.excel.MgUploadValidatorHldtax;
 ...
 */
@ApplicationScoped
public class Mg0210Service extends BasePagingService {

	@Inject private Mg0210Repository repository;
	@Inject private MgUploadValidatorHldtax validator;
	@Inject private MgExcelReaderHldtax reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookHldtax> bookMap = new HashMap<String, MgExcelBookHldtax>();
	 ...
	 * @return
	 */
	public Mg0210Response init(Mg0210Request req) {
		final Mg0210Response res = createResponse(Mg0210Response.class, req);
	 ...
	 * @return
	 */
	public Mg0210Response search(Mg0210Request req) {
		final int allCount = repository.count(req);
		final Mg0210Response res = createResponse(Mg0210Response.class, req, allCount);
	 ...
	 * @return
	 */
	public Mg0210Response delete(Mg0210Request req) {
		final Mg0210Response res = createResponse(Mg0210Response.class, req);
		if (req.checkeds!=null && req.checkeds.size()>0) {
			for (HldtaxMstPK id : req.checkeds) {
	...
	public Response download(Mg0210Request req) {
		final LoginInfo u = sessionHolder.getLoginInfo();
		final String corporationCode = u.getCorporationCode();
		final String fileName = MessageFormat.format("源泉税マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookHldtax book = new MgExcelBookHldtax();

		book.sheet = new MgExcelSheetHldtax();
		book.sheet.entityList = repository.getMasterData((Mg0210Request) req);

		final MgUploadSOHldtax streaming = new MgUploadSOHldtax(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOHldtax streaming = new MgUploadSOHldtax(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookHldtax book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookHldtax book) throws IOException {
