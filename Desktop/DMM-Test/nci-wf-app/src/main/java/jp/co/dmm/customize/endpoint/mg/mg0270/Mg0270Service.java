...
import jp.co.dmm.customize.endpoint.mg.mg0270.excel.MgExcelReaderBgtItm;
import jp.co.dmm.customize.endpoint.mg.mg0270.excel.MgExcelSheetBgtItm;
import jp.co.dmm.customize.endpoint.mg.mg0270.excel.MgUploadSOBgtItm;
import jp.co.dmm.customize.endpoint.mg.mg0270.excel.MgUploadValidatorBgtItm;
 ...
 */
@ApplicationScoped
public class Mg0270Service extends BasePagingService  {


	@Inject private Mg0270Repository repository;
	@Inject private MgUploadValidatorBgtItm validator;
	@Inject private MgExcelReaderBgtItm reader;
	@Inject private Logger log;
	@Inject private MwmLookupService lookup;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookBgtItm> bookMap = new HashMap<String, MgExcelBookBgtItm>();
	 ...
	 * @return
	 */
	public Mg0270SearchResponse init(Mg0270SearchRequest req) {
		final Mg0270SearchResponse res = createResponse(Mg0270SearchResponse.class, req);
	 ...
	 * @return
	 */
	public Mg0270SearchResponse search(Mg0270SearchRequest req) {
		...
		final Mg0270SearchResponse res = createResponse(Mg0270SearchResponse.class, req, allCount);
	 ...
	 * @return
	 */
	public Mg0270RemoveResponse delete(Mg0270RemoveRequest req) {
		final Mg0270RemoveResponse res = createResponse(Mg0270RemoveResponse.class, req);
		int delTotalCnt = repository.delete(req);
		res.success = (delTotalCnt > 0);
		return res;
	}

	public Response download(Mg0270SearchRequest req) {
		final LoginInfo u = sessionHolder.getLoginInfo();
		final String corporationCode = u.getCorporationCode();
		final String fileName = MessageFormat.format("予算科目マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookBgtItm book = new MgExcelBookBgtItm();

		book.sheet = new MgExcelSheetBgtItm();
		book.sheet.entityList = repository.getMasterData((Mg0270SearchRequest) req);

		final MgUploadSOBgtItm streaming = new MgUploadSOBgtItm(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOBgtItm streaming = new MgUploadSOBgtItm(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookBgtItm book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookBgtItm book) throws IOException {
