 ...
 */
@BizLogic
public class Rm0720Service extends MmBaseService<WfmMenuRoleDetail> {
	 ...
	 * @return
	 */
	public Rm0720Response init(Rm0720Request req) {
		if (isEmpty(req.corporationCode) || isEmpty(req.menuRoleCode))
			throw new BadRequestException("企業コードまたは利用者ロールコードが未指定です");

		final Rm0720Response res = createResponse(Rm0720Response.class, req);
...
//		res.menuRoleDetailList = sardOut.getMenuRoleDetails();
	...
	public Rm0720Response search(Rm0720Request req) {
		...
		final Rm0720Response res = createResponse(Rm0720Response.class, req, allCount);
	 ...
	 */
	@Transactional
	public Rm0720Response update(Rm0720UpdateRequest req) {
		final Rm0720Response res = createResponse(Rm0720Response.class, req);

		// 企業管理者ロールは削除不可
		if (eq(MenuRoleCodes.CorpAdmin, req.menuRole.getMenuRoleCode())
				&& eq(DeleteFlag.ON, req.menuRole.getDeleteFlag())) {
			res.addAlerts(i18n.getText(MessageCd.MSG0160));
			res.success = false;
		}
		else {
			// 利用者ロール取得
	 ...
	 */
	@Transactional
	public Rm0720Response delete(Rm0720Request req) {
