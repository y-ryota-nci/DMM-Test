...
/**
 * メールテンプレート定義用ストリーム書き込みクラス
 */
@ApplicationScoped
public class MailTemplateDownloader extends MiscUtils implements StreamingOutput, CodeBook {
	@Inject private DownloadNotifyService notify;
	@Inject private MailTemplateDownloadService service;

	/** 企業コード */
	private String corporationCode;

	/**
	 * ダウンロードの下準備。実際の処理は MailTemplateDownloader.write() で行われる
	...
	public void write(OutputStream output) throws IOException, WebApplicationException {
		...
		try (ZipCompressor zip = new ZipCompressor(output, MS932)) {
			...
			final MailTemplateDownloadDto dto = service.createDto(corporationCode);
			if (dto != null) {
