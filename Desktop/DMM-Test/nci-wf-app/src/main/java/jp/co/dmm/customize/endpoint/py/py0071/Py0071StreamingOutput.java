...
public class Py0071StreamingOutput extends MiscUtils implements StreamingOutput {
	private static final String SHEETNAME_PAYABLE_BALANCE_DETAIL = "買掛残高詳細";
	/** 処理結果Excel */
	Py0071Book book;
	 ...
	 * @param book
	 */
	public Py0071StreamingOutput(Py0071Book book) {
		this.book = book;
	}
	...
	public void write(OutputStream output) throws IOException, WebApplicationException {
		...
		final ClassPathResource cpr = new ClassPathResource("excel/py0071.xlsx");
	...
	private void writeWorkbook(BufferedInputStream bis, OutputStream output)
		...
		final Py0071ExcelWriter writer = CDI.current().select(Py0071ExcelWriter.class).get();
		try (Workbook wb = WorkbookFactory.create(bis)) {
			List<String> removeSheetName = new ArrayList<String>();
			if (book.sheetPayableBalDtl==null) removeSheetName.add(SHEETNAME_PAYABLE_BALANCE_DETAIL);
			final Map<Integer, CellStyle> styles = new HashMap<>();
			for (Sheet sheet : wb) {
				final String name = sheet.getSheetName();
				if (eq(SHEETNAME_PAYABLE_BALANCE_DETAIL, name))
					writer.writePayableBalDtl(sheet, book.sheetPayableBalDtl, styles);
			}
			wb.write(output);
		}
	}
