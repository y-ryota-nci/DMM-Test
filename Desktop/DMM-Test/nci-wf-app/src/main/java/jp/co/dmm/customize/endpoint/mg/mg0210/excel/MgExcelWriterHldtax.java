...
/**
 * 源泉税情報アップロードのEXCELファイルのライター
 */
@ApplicationScoped
public class MgExcelWriterHldtax extends MgExcelWriter<MgExcelSheetHldtax> {

	/** 源泉税マスタシートの書き込み */
	@Override
	public void writeMaster(Sheet sheet, MgExcelSheetHldtax sheetObj, Map<Integer, CellStyle> styles) {
		final int START = 2;
		int r = START;
		for (MgExcelEntityHldtax entity : sheetObj.entityList) {
			// 次行を生成＆現行行の書式をコピー
			...
			setCellValue(row, i++, entity.hldtaxTp);
			setCellValue(row, i++, entity.hldtaxNm);
			setCellValue(row, i++, entity.hldtaxRto1);
			setCellValue(row, i++, entity.hldtaxRto2);
