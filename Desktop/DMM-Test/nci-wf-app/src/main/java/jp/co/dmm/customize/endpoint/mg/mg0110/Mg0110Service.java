...
import jp.co.dmm.customize.endpoint.mg.mg0110.excel.MgExcelReaderBnkbrc;
import jp.co.dmm.customize.endpoint.mg.mg0110.excel.MgExcelSheetBnkbrc;
import jp.co.dmm.customize.endpoint.mg.mg0110.excel.MgUploadSOBnkbrc;
import jp.co.dmm.customize.endpoint.mg.mg0110.excel.MgUploadValidatorBnkbrc;
...
@ApplicationScoped
public class Mg0110Service extends MgUploadService  {


	@Inject private Mg0110Repository repository;
	@Inject private MgUploadValidatorBnkbrc validator;
	@Inject private MgExcelReaderBnkbrc reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookBnkbrc> bookMap = new HashMap<String, MgExcelBookBnkbrc>();
	 ...
	 * @return
	 */
	public Mg0110SearchResponse init(Mg0110SearchRequest req) {
		final Mg0110SearchResponse res = createResponse(Mg0110SearchResponse.class, req);
	 ...
	 * @return
	 */
	public Mg0110SearchResponse search(Mg0110SearchRequest req) {
		...
		final Mg0110SearchResponse res = createResponse(Mg0110SearchResponse.class, req, allCount);
	 ...
	 * @return
	 */
	public Mg0110Response delete(Mg0110Request req) {
		final Mg0110Response res = createResponse(Mg0110Response.class, req);
	...
	public Response download(BasePagingRequest req) {
		...
		final String fileName = MessageFormat.format("銀行支店マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookBnkbrc book = new MgExcelBookBnkbrc();

		book.sheet = new MgExcelSheetBnkbrc();
		book.sheet.entityList = repository.getMasterData((Mg0110SearchRequest) req);

		final MgUploadSOBnkbrc streaming = new MgUploadSOBnkbrc(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOBnkbrc streaming = new MgUploadSOBnkbrc(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookBnkbrc book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookBnkbrc book) throws IOException {
