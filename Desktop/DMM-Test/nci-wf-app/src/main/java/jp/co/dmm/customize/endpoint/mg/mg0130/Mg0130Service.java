...
import jp.co.dmm.customize.endpoint.mg.mg0130.excel.MgExcelReaderAcc;
import jp.co.dmm.customize.endpoint.mg.mg0130.excel.MgExcelSheetAcc;
import jp.co.dmm.customize.endpoint.mg.mg0130.excel.MgUploadSOAcc;
import jp.co.dmm.customize.endpoint.mg.mg0130.excel.MgUploadValidatorAcc;
...
@ApplicationScoped
public class Mg0130Service extends BasePagingService  {


	@Inject private Mg0130Repository repository;
	@Inject private MgUploadValidatorAcc validator;
	@Inject private MgExcelReaderAcc reader;
	@Inject private Logger log;

	// アップロードファイルデータマップ
	Map<String, MgExcelBookAcc> bookMap = new HashMap<String, MgExcelBookAcc>();
	 ...
	 * @return
	 */
	public Mg0130SearchResponse init(Mg0130SearchRequest req) {
		final Mg0130SearchResponse res = createResponse(Mg0130SearchResponse.class, req);
	 ...
	 * @return
	 */
	public Mg0130SearchResponse search(Mg0130SearchRequest req) {
		...
		final Mg0130SearchResponse res = createResponse(Mg0130SearchResponse.class, req, allCount);
	 ...
	 * @return
	 */
	public Mg0130Response delete(Mg0130Request req) {
		final Mg0130Response res = createResponse(Mg0130Response.class, req);
		int delTotalCnt = repository.delete(req);
		res.success = (delTotalCnt > 0);
		return res;
	}

	public Response download(Mg0130SearchRequest req) {
		final LoginInfo u = sessionHolder.getLoginInfo();
		final String corporationCode = u.getCorporationCode();
		final String fileName = MessageFormat.format("勘定科目マスタ_{0}_{1}.xlsx", corporationCode, toStr(today(), "yyyyMMdd"));

		MgExcelBookAcc book = new MgExcelBookAcc();

		book.sheet = new MgExcelSheetAcc();
		book.sheet.entityList = repository.getMasterData((Mg0130SearchRequest) req);

		final MgUploadSOAcc streaming = new MgUploadSOAcc(book);
			...
			try {
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(xlsBytes)) {
					...
					if (validator.validate(book)) {
						...
						MgUploadSOAcc streaming = new MgUploadSOAcc(book);
	...
	public Response register(MgUploadSaveRequest req) {
		try {
			...
			final MgExcelBookAcc book = bookMap.get(req.encoded);
	...
	private void parse(InputStream stream, MgExcelBookAcc book) throws IOException {
