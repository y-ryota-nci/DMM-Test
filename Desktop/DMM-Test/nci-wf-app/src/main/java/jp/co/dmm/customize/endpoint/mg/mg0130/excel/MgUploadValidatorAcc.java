...
import jp.co.dmm.customize.endpoint.mg.mg0131.Mg0131Repository;
import jp.co.dmm.customize.endpoint.mg.mg0131.Mg0131UpdateRequest;
 ...
 */
@ApplicationScoped
public class MgUploadValidatorAcc extends MgUploadValidator<MgExcelBookAcc> {
	...
	/** 勘定科目マスタのバリデーション */
	@Override
	protected int validateMaster(MgExcelBookAcc book) {

		int allErrors = 0;

		// 会社コード、通貨コードをキーにMap化
		final Map<String, List<MgExcelEntityAcc>> map = book.sheet.entityList.stream().collect(Collectors.groupingBy(d -> toKey(d)));
		for (String key : map.keySet()) {
			final List<MgExcelEntityAcc> list = map.get(key);

			for (MgExcelEntityAcc entity : list) {
				// 入力チェック
				...
				if (errors == 0) {
					// 存在チェック
					AccMst dbEntity = repository.getByPk(entity.companyCd, entity.accCd, entity.sqno);
					if (eq("U", entity.processType)) {
						entity.processType = dbEntity != null ? "C" : "A";
					}

					if (eq("A", entity.processType) && dbEntity != null) {
						entity.errorText = "勘定科目コード情報はデータベース内の同一が重複しています。";
						errors ++;
					} else if (in(entity.processType, "C", "D") && dbEntity == null) {
						entity.errorText = "勘定科目コード情報はデータベース内の存在していません。";
						errors ++;
					}
				}
				...
				if (errors == 0) {
					...
					if (in(entity.processType, "A", "C")) {
						Mg0131UpdateRequest req = new Mg0131UpdateRequest();
						...
						if (maxSqnoOverlap > 0) {
							entity.errorText = "勘定科目コード情報はデータベースにおいて有効期間が重複しています。";
							errors ++;
						}
					}
				}
	...
	private String toKey(MgExcelEntityAcc data) {
		return join(data.companyCd, "_", data.accCd);
	}

	/** データ行の単項目単位のバリデーション. */
	private int validateBase(MgExcelBookAcc book, MgExcelEntityAcc entity) {
		int allErrors = 0;
		...
		if (in(entity.processType, "A", "U", "C")) {
			fields.addField(entity.accNm, "勘定科目名称")
				.required()
				.anyString(60);

			fields.addField(entity.accNmS, "勘定科目略称")
				.anyString(30);

			fields.addField(entity.dcTp, "貸借区分")
				.include(book.dcTps);

			fields.addField(entity.accBrkDwnTp, "勘定科目補助区分")
				.include(book.accBrkdwnTps);

			fields.addField(entity.taxCdSs, "税処理コード（SuperStream）")
				.alphaNumeric(4);

			fields.addField(entity.taxIptTp, "税入力区分")
				.include(book.taxIptTps);
