$(function() {
	const pager = new Pager($('#seach-result'), '/mg0100/search', search).init();

	const params = { messageCds : [] };
	NCI.init("/mg0100/init", params).done(function(res) {
		if (res && res.success) {
			...
			.on('click', '#btnDelete', function() {
				...
				NCI.confirm(msg, function() {
					...
					if (target) {
						...
						NCI.init("/mg0100/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = '銀行情報を削除しました。';
								$('#mg0100InformationContents').html(msg);
								$('#mg0100Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0100/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0100/upload", false, displayUploadResult);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0101.html?insertFlg=true");
	}

	/** 明細行（銀行マスタ）を開く */
	function openEntry(companyCd, bnkCd) {
		NCI.redirect("./mg0101.html?companyCd=" + companyCd + "&bnkCd=" + bnkCd);
	}
	...
	function uploadRegister(ev) {
		...
		let msg = '銀行情報を登録します。よろしいですか？';
		NCI.confirm(msg, function() {
			const params = {
				encoded : $('#encoded').val(),
				fileName : $('#fileName').text()
			};
			NCI.post('/mg0100/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0100/download', cond);
	}
});
