$(function() {
	const pager = new Pager($('#seach-result'), '/mg0240/search', search).init();

	const params = { messageCds : [] };
	NCI.init("/mg0240/init", params).done(function(res) {
		if (res && res.success) {
			...
			NCI.createOptionTags($('#paySiteM'), res.paySiteMOpts);
			NCI.createOptionTags($('#paySiteN'), res.paySiteNOpts);
			...
			.on('click', 'a[data-field]', function(ev) {
				...
				var paySiteCd = $('>.first>.paySiteCd', $(this).parent().parent()).val();

				openEntry(companyCd, paySiteCd);
			})
			...
			.on('click', '#btnDelete', function() {
				let msg = '支払サイトマスタを削除します。よろしいですか？';
				NCI.confirm(msg, function() {
					...
					for(var i=0; i<$('#seach-result .selectable').size(); i++) {
						if ($('#seach-result .selectable').get(i).checked) {
							...
							var paySiteCd = $('>.first>.paySiteCd', $(checkTarget).parent().parent()).val();

							if (targetCnt != 0) {
								target += ',';
							}

							target += companyCd + '|' + paySiteCd;
							targetCnt++;
						}
					}
					...
					if (target) {
						...
						NCI.init("/mg0240/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = '支払サイトマスタを削除しました。';
								$('#mg0240InformationContents').html(msg);
								$('#mg0240Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0240/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0240/upload", false, displayUploadResult);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0241.html?insertFlg=true");
	}

	/** 明細行（銀行マスタ）を開く */
	function openEntry(companyCd, paySiteCd) {
		NCI.redirect("./mg0241.html?companyCd=" + companyCd + "&paySiteCd=" + paySiteCd);
	}
	...
	function uploadRegister(ev) {
		...
		let msg = '支払サイト情報を登録します。よろしいですか？';
		NCI.confirm(msg, function() {
			const params = {
				encoded : $('#encoded').val(),
				fileName : $('#fileName').text()
			};
			NCI.post('/mg0240/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0240/download', cond);
	}
