$(function() {
	const pager = new Pager($('#seach-result'), '/mg0220/search', search).init();

	const params = { messageCds : [] };
	NCI.init("/mg0220/init", params).done(function(res) {
		if (res && res.success) {
			...
			.on('click', 'a[data-field]', function(ev) {
				...
				var payApplCd = $('>.first>.payApplCd', $(this).parent().parent()).val();

				openEntry(companyCd, payApplCd);
			})
			// 行選択
				...
				NCI.confirm(msg, function() {
					...
					for(var i=0; i<$('#seach-result .selectable').size(); i++) {
						if ($('#seach-result .selectable').get(i).checked) {
							...
							var payApplCd = $('>.first>.payApplCd', $(checkTarget).parent().parent()).val();

							if (targetCnt != 0) {
								target += ',';
							}

							target += companyCd + '|' + payApplCd;
							targetCnt++;
						}
					}
					...
					if (target) {
						...
						NCI.init("/mg0220/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = '支払業務マスタを削除しました。';
								$('#mg0220InformationContents').html(msg);
								$('#mg0220Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0220/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0220/upload", false, displayUploadResult);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0221.html?insertFlg=true");
	}

	/** 明細行を開く */
	function openEntry(companyCd, payApplCd) {
		NCI.redirect("./mg0221.html?companyCd=" + companyCd + "&payApplCd=" + payApplCd);
	}
	...
	function uploadRegister(ev) {
		...
		let msg = '支払業務情報を登録します。よろしいですか？';
		NCI.confirm(msg, function() {
			const params = {
				encoded : $('#encoded').val(),
				fileName : $('#fileName').text()
			};
			NCI.post('/mg0220/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0220/download', cond);
	}
