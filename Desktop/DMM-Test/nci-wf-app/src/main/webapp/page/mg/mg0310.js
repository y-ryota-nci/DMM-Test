$(function() {
	const pager = new Pager($('#seach-result'), '/mg0310/search', search).init();

	const params = { messageCds : ['MSG0208'] };
	NCI.init("/mg0310/init", params).done(function(res) {
		if (res && res.success) {
			...
			.on('click', 'a[data-field]', function(ev) {
				...
				var zipCd = $('>.first>.zipCd', $(this).parent().parent()).val();
				var sqno = $('>.first>.sqno', $(this).parent().parent()).val();

				openEntry(companyCd, zipCd, sqno);
			})
				...
				NCI.confirm(msg, function() {
					...
					for(var i=0; i<$('#seach-result .selectable').size(); i++) {
						if ($('#seach-result .selectable').get(i).checked) {
							...
							var zipCd = $('>.first>.zipCd', $(checkTarget).parent().parent()).val();
					...
					if (target) {
						...
						NCI.init("/mg0310/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = '住所マスタを削除しました。';
								$('#mg0310InformationContents').html(msg);
								$('#mg0310Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0310/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0310/upload", false, displayUploadResult);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0311.html?insertFlg=true");
	}

	/** 明細行を開く */
	function openEntry(companyCd, zipCd, sqno) {
		NCI.redirect("./mg0311.html?companyCd=" + companyCd + "&zipCd=" + zipCd + "&sqno=" + sqno);
	}
	...
	function uploadRegister(ev) {
		...
		NCI.confirm(msg, function() {
			...
			NCI.post('/mg0310/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0310/download', cond);
	}
