$(function() {
	const pager = new Pager($('#seach-result'), '/mg0280/search', search).init();

	const params = { messageCds : [] };
	NCI.init("/mg0280/init", params).done(function(res) {
		if (res && res.success) {
			...
			.on('click', 'a[data-field]', function(ev) {
				...
				var mdaId = $('>.first>.mdaId', $(this).parent().parent()).val();

				openEntry(companyCd, mdaId);
			})
			// 行選択
				...
				NCI.confirm(msg, function() {
					...
					for(var i=0; i<$('#seach-result .selectable').size(); i++) {
						if ($('#seach-result .selectable').get(i).checked) {
							...
							var mdaId = $('>.first>.mdaId', $(checkTarget).parent().parent()).val();

							if (targetCnt != 0) {
								target += ',';
							}

							target += companyCd + '|' + mdaId;
							targetCnt++;
						}
					}
					...
					if (target) {
						...
						NCI.init("/mg0280/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = 'メディアマスタを削除しました。';
								$('#mg0280InformationContents').html(msg);
								$('#mg0280Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0280/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0280/upload", false, displayUploadResult);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0281.html?insertFlg=true");
	}

	/** 明細行を開く */
	function openEntry(companyCd, mdaId) {
		NCI.redirect("./mg0281.html?companyCd=" + companyCd + "&mdaId=" + mdaId);
	}
	...
	function uploadRegister(ev) {
		...
		let msg = 'メディア情報を登録します。よろしいですか？';
		NCI.confirm(msg, function() {
			const params = {
				encoded : $('#encoded').val(),
				fileName : $('#fileName').text()
			};
			NCI.post('/mg0280/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0280/download', cond);
	}
