$(function() {
	const pager = new Pager($('#seach-result'), '/mg0160/search', search).init();

	const params = { messageCds : [] };
	NCI.init("/mg0160/init", params).done(function(res) {
		if (res && res.success) {
			...
			.on('click', 'a[data-field]', function(ev) {
				...
				var taxCd = $('>.first>.taxCd', $(this).parent().parent()).val();
				var sqno = $('>.first>.sqno', $(this).parent().parent()).val();

				openEntry(companyCd, taxCd, sqno);
			})
				...
				NCI.confirm(msg, function() {
					...
					for(var i=0; i<$('#seach-result .selectable').size(); i++) {
						if ($('#seach-result .selectable').get(i).checked) {
							...
							var taxCd = $('>.first>.taxCd', $(checkTarget).parent().parent()).val();
					...
					if (target) {
						...
						NCI.init("/mg0160/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = '消費税マスタを削除しました。';
								$('#mg0160InformationContents').html(msg);
								$('#mg0160Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0160/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0160/upload", false, displayUploadResult);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0161.html?insertFlg=true");
	}

	/** 明細行を開く */
	function openEntry(companyCd, taxCd, sqno) {
		NCI.redirect("./mg0161.html?companyCd=" + companyCd + "&taxCd=" + taxCd + "&sqno=" + sqno);
	}
	...
	function uploadRegister(ev) {
		...
		let msg = '消費税情報を登録します。よろしいですか？';
		NCI.confirm(msg, function() {
			const params = {
				encoded : $('#encoded').val(),
				fileName : $('#fileName').text()
			};
			NCI.post('/mg0160/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0160/download', cond);
	}
