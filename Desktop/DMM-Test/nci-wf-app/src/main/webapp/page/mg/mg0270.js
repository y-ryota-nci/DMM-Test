$(function() {
	const pager = new Pager($('#seach-result'), '/mg0270/search', search).init();

	const params = { messageCds : [] };
	NCI.init("/mg0270/init", params).done(function(res) {
		if (res && res.success) {
			...
			// BS/PL区分
			NCI.createOptionTags($('#bsPlTp'), res.bsPlTps);
			...
			.on('click', 'a[data-field]', function(ev) {
				...
				var bgtItmCd = $('>.first>.bgtItmCd', $(this).parent().parent()).val();

				openEntry(companyCd, bgtItmCd);
			})
			// 行選択
				...
				NCI.confirm(msg, function() {
					...
					for(var i=0; i<$('#seach-result .selectable').size(); i++) {
						if ($('#seach-result .selectable').get(i).checked) {
							...
							var bgtItmCd = $('>.first>.bgtItmCd', $(checkTarget).parent().parent()).val();

							if (targetCnt != 0) {
								target += ',';
							}

							target += companyCd + '|' + bgtItmCd;
							targetCnt++;
						}
					}
					...
					if (target) {
						...
						NCI.init("/mg0270/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = '予算科目マスタを削除しました。';
								$('#mg0270InformationContents').html(msg);
								$('#mg0270Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0270/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0270/upload", false, displayUploadResult);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0271.html?insertFlg=true");
	}

	/** 明細行を開く */
	function openEntry(companyCd, bgtItmCd) {
		NCI.redirect("./mg0271.html?companyCd=" + companyCd + "&bgtItmCd=" + bgtItmCd);
	}
	...
	function uploadRegister(ev) {
		...
		let msg = '予算科目情報を登録します。よろしいですか？';
		NCI.confirm(msg, function() {
			const params = {
				encoded : $('#encoded').val(),
				fileName : $('#fileName').text()
			};
			NCI.post('/mg0270/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0270/download', cond);
	}
