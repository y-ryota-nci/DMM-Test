$(function() {
	const pager = new Pager($('#seach-result'), '/mg0170/search', search).init();

	const params = { messageCds : [] };
	NCI.init("/mg0170/init", params).done(function(res) {
		if (res && res.success) {
			...
			.on('click', 'a[data-field]', function(ev) {
				...
				var mnyCd = $('>.first>.mnyCd', $(this).parent().parent()).val();

				openEntry(companyCd, mnyCd);
			})
				...
				NCI.confirm(msg, function() {
					...
					for(var i=0; i<$('#seach-result .selectable').size(); i++) {
						if ($('#seach-result .selectable').get(i).checked) {
							...
							var mnyCd = $('>.first>.mnyCd', $(checkTarget).parent().parent()).val();

							if (targetCnt != 0) {
								target += ',';
							}

							target += companyCd + '|' + mnyCd;
							targetCnt++;
						}
					}
					...
					if (target) {
						...
						NCI.init("/mg0170/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = '通貨マスタを削除しました。';
								$('#mg0170InformationContents').html(msg);
								$('#mg0170Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0170/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0170/upload", false, displayUploadResult);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0171.html?insertFlg=true");
	}

	/** 明細行（銀行マスタ）を開く */
	function openEntry(companyCd, mnyCd) {
		NCI.redirect("./mg0171.html?companyCd=" + companyCd + "&mnyCd=" + mnyCd);
	}
	...
	function uploadRegister(ev) {
		...
		let msg = '通貨情報を登録します。よろしいですか？';
		NCI.confirm(msg, function() {
			const params = {
				encoded : $('#encoded').val(),
				fileName : $('#fileName').text()
			};
			NCI.post('/mg0170/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0170/download', cond);
	}
});
