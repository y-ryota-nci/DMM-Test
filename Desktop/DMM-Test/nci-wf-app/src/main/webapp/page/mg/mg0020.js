$(function() {
	const pager = new Pager($('#seach-result'), '/mg0020/search', search).init();

	const params = { messageCds : [] };
	NCI.init("/mg0020/init", params).done(function(res) {
		if (res && res.success) {
			...
			.on('click', 'a[data-field]', function(ev) {
				...
				var itmexpsCd = $('>.first>.itmexpsCd', $(this).parent().parent()).val();

				openEntry(companyCd, itmexpsCd);
			})
				...
				NCI.confirm(msg, function() {
					...
					for(var i=0; i<$('#seach-result .selectable').size(); i++) {
						if ($('#seach-result .selectable').get(i).checked) {
							...
							var itmexpsCd = $('>.first>.itmexpsCd', $(checkTarget).parent().parent()).val();

							if (targetCnt != 0) {
								target += ',';
							}

							target += companyCd + '|' + itmexpsCd;
							targetCnt++;
						}
					}
					...
					if (target) {
						...
						NCI.init("/mg0020/delete", params).done(function(res) {
							if (res && res.success) {
								var msg = '費目情報を削除しました。';
								$('#mg0020InformationContents').html(msg);
								$('#mg0020Information').modal({show: true});
								search(1, true);
			...
			FileUploader.setup("div.dragZone", "/mg0020/upload", false, displayUploadResult);

			// ファイルコントロールによるファイルアップロード
			FileUploader.setup('input[type=file]', "/mg0020/upload", false, displayUploadResult);
	...
	function callbackFromSelectCompany(company) {
		if (company) {
			$('#companyCd').val(company['CORPORATION_CODE']);
	...
	function openEntryForInsert() {
		NCI.redirect("./mg0021.html?insertFlg=true");
	}

	/** 明細行（費目マスタ）を開く */
	function openEntry(companyCd, itmexpsCd) {
		NCI.redirect("./mg0021.html?companyCd=" + companyCd + "&itmexpsCd=" + itmexpsCd);
	}
	...
	function uploadRegister(ev) {
		...
		let msg = '費目情報を登録します。よろしいですか？';
		NCI.confirm(msg, function() {
			const params = {
				encoded : $('#encoded').val(),
				fileName : $('#fileName').text()
			};
			NCI.post('/mg0020/register', params).done(function(res) {
				uploadReset();
	...
	function download() {
		...
		NCI.download('/mg0020/download', cond);
	}
});
