$(function() {
	let pager = new Pager($('#seach-result'), '/ti0020/search', search).init();
	NCI.init("/ti0020/init", {messageCds: []}).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
	...
	.on('click', 'a[data-field]', function(ev) {
		...
		let menuRoleCode = $tr.find('[data-field=menuRoleCode]').text();
