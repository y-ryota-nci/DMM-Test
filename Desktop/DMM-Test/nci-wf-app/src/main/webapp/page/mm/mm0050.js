$(function() {
	var pager = new Pager($('#seach-result'), '/mm0050/search', search).init();

	var params = {
		messageCds : []
	};
	NCI.init("/mm0050/init", params).done(function(res) {
		if (res && res.success) {
	...
	.on('click', 'a[data-field]', function(ev) {
		...
		var partsNumberingFormatCode = $tr.find('[data-field=partsNumberingFormatCode]').text();
		var version = $tr.find('[data-field=version]').text();
		openEntry(corporationCode, partsNumberingFormatCode, version);
	})
	// 追加ボタン
	.on('click', '#btnAdd', function(ev) {
		NCI.redirect("./mm0051.html?corporationCode=" + NCI.loginInfo.corporationCode);
	})
	;
	...
	function createCondition(pageNo) {
		...
		// デフォルトソート条件
		if (!cond.sortColumn) {
			cond.sortColumn = 'PARTS_NUMBERING_FORMAT_CODE, PARTS_NUMBERING_FORMAT_ID';
			cond.sortAsc = true;
		}
		return cond;
	}

	function openEntry(corporationCode, partsNumberingFormatCode, version) {
		NCI.redirect("./mm0051.html?corporationCode=" + corporationCode +
				"&partsNumberingFormatCode=" + partsNumberingFormatCode +
				"&version=" + version);
	}
});
