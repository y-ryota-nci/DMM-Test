$(function() {
	...
	const tray = new TRAY('/wl0031/search', $('#formCondition'), $('#seach-result'), $('#tray-template'), search);
	const params = { "messageCds" : ['sendback', 'pullback', 'pullforward']};
	NCI.init('/wl0031/init', params).done(function(res) {
		if (res && res.success) {
	...
	function downloadCsv(ev) {
		...
		NCI.download('/wl0031/downloadCsv', cond);
