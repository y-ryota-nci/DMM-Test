$(function() {
	const pager = new Pager($('#seach-result'), '/wl0015/search', search).init();
	pager.responsiveTable.modifyTR = modifyTR;	// 標準機能を上書き

	const params = { messageCds : ['MSG0120', 'trayConfig'] };
	NCI.init("/wl0015/init", params).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
			...
			.on('click', '#btnDelete', function(ev) {
				...
				if (NCI.confirm(msg, function() {
					...
					NCI.post("/wl0015/delete", params).done(function(res) {
						search(1, true);
					});
				}));
			})
	...
	function openEntry(trayConfigId, version) {
		let url = "./wl0011.html?from=wl0015";
		if (trayConfigId) {
