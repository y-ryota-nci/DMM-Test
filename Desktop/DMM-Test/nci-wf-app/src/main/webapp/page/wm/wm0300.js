$(function() {
	var pager = new Pager($('#seach-result'), '/wm0300/search', search).init();

	var params = {
		messageCds : []
	};
	NCI.init("/wm0300/init", params).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
	...
	.on('click', 'a[data-field]', function(ev) {
		...
		openEntry(corporationCode, changeRoleCode);
	})
	// 追加ボタン
	.on('click', '#btnAdd', function(ev) {
		NCI.redirect("./wm0310.html?corporationCode=" + NCI.loginInfo.corporationCode);
	...
	function createCondition(pageNo) {
		...
		// デフォルトソート条件
		if (!cond.sortColumn) {
			cond.sortColumn = 'CHANGE_ROLE_CODE';
			cond.sortAsc = true;
		}
		return cond;
	}
