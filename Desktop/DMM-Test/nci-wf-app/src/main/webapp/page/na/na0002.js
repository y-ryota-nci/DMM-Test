$(function() {
	var $treeScreenProcessLevelDef = $('#treeScreenProcessLevelDef');
	$treeScreenProcessLevelDef.jstree();
	...
	var init = function(res) {
		...
		pager.fillTable(res.accessibleScreens);
		NCI.ymdPicker($('#searchResult tbody').find('input.ymdPicker'));

		refreshTree(res.treeItems);
		var checked = $('#tblAccessibleScreens tbody input[type=checkbox]:checked').length === 0;
		$('#btnUpdate,#btnRemove').prop('disabled', checked);
	};

	var refreshTree = function(treeItems) {
		$treeScreenProcessLevelDef.jstree().destroy();
		$treeScreenProcessLevelDef
		.on('ready.jstree', function(ev, instance) {
			refreshTreeItem($treeScreenProcessLevelDef.jstree().get_json('#'));
		})
		.on('changed.jstree', function(e, data) {
			var selected = $treeScreenProcessLevelDef.jstree().get_selected();
	...
	var refreshTreeItem = function(children) {
		...
		for (var i in children) {
			...
			if (node.type === 'book' && $treeScreenProcessLevelDef.jstree().is_leaf(node.id)) $treeScreenProcessLevelDef.jstree().delete_node(node.id);
		}
	};

	NCI.init('/na0002/init', params).done(function(res) {
		if (res && res.success) {
			if (res.menuRole) {
			...
			$(document)
			.on('change', '#tblAccessibleScreens tbody input[type=checkbox]', function() {
				var checked = $('#tblAccessibleScreens tbody input[type=checkbox]:checked').length === 0;
				$('#btnUpdate,#btnRemove').prop('disabled', checked);
			})
			...
			.on('click', '#btnUpdate', function(ev) {
				var checked = $('#tblAccessibleScreens tbody input[type=checkbox]:checked');
				...
				NCI.confirm(msg, function() {
					var accessibleScreens = [];
					$.each(checked, function(i, e) {
						var $tr = $(e).parent().parent();
						var screenProcessId = $tr.find('[data-field=screenProcessId]').text();
						var accessibleScreenId = $tr.find('[data-field=accessibleScreenId]').text();
						var validStartDate = $tr.find('[data-field=validStartDate]').val();
						var validEndDate = $tr.find('[data-field=validEndDate]').val();
						var newApplyDisplayFlag = $tr.find('[data-field=newApplyDisplayFlag]').val();
						accessibleScreens.push({
							corporationCode : corporationCode,
							menuRoleCode : menuRoleCode,
							screenProcessId : screenProcessId,
							accessibleScreenId: accessibleScreenId,
							validStartDate: validStartDate,
							validEndDate: validEndDate,
							newApplyDisplayFlag: newApplyDisplayFlag
						});
					...
					NCI.post('/na0002/update', params).done(function(res) {
						if (res && res.success) {
							init(res);
						}
					});
			...
			.on('click', '#btnRemove', function(ev) {
				...
				NCI.confirm(msg, function() {
					...
					$.each(checked, function(i, e) {
						var $tr = $(e).parent().parent();
						var accessibleScreenId = $tr.find('[data-field=accessibleScreenId]').text();
						accessibleScreens.push({accessibleScreenId: accessibleScreenId});
					});
					var params = {
						corporationCode: corporationCode,
						menuRoleCode: menuRoleCode,
					...
					NCI.post('/na0002/remove', params).done(function(res) {
						if (res && res.success) {
							init(res);
						}
					});
			...
			.on('click', '#btnOK', function(e) {
				...
				$.each(selected, function(i, e) {
					var node = $treeScreenProcessLevelDef.jstree().get_node(e);
					if (node.type === 'file') {
						accessibleScreens.push({
							corporationCode : corporationCode,
							menuRoleCode : menuRoleCode,
							screenProcessId: node.data.screenProcessId,
							validStartDate: node.data.validStartDate,
							validEndDate: node.data.validEndDate,
							newApplyDisplayFlag: node.data.newApplyDisplayFlag
						});
					}
				});
				...
				NCI.post('/na0002/create', params).done(function(res) {
					if (res && res.success) {
						init(res);
					}
