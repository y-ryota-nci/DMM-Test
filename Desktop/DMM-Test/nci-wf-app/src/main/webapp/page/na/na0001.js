$(function() {
	var pager = new Pager($('#seach-result'), '/na0001/search', search).init();
	NCI.init("/na0001/init", {messageCds: []}).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
	...
	function openEntry(corporationCode, menuRoleCode) {
		NCI.redirect("./na0002.html?corporationCode=" + corporationCode + "&menuRoleCode=" + menuRoleCode);
	}
});
