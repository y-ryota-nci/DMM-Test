...
$(function() {
	...
	// ページング情報(口座情報)
	pager = new Pager($('#seach-result'), '/md0011/getAccountList', md0011.getAccountList).init();
	...
	.on('click', '#btnUpdate', function() {
		...
		NCI.confirm(messege, function() {
			...
			// 更新処理
			var params = md0011.getSubmitValue();

			// 初期化処理開始
			NCI.init("/md0011/update", params).done(function(res, textStatus, jqXHR) {
				if (res && res.success) {
					// 完了メッセージ
					var msg = '登録内容の更新が完了しました。';
					$('#md0011InformationContents').html(msg);
					$('#md0011Information').modal({show: true});
				}
			});
		});
	});
...
var md0011 = {
	...
	// 初期化処理
	init : function () {
		NCI.init("/md0011/init", md0011.searchParam).done(function(res, textStatus, jqXHR) {
			if (res && res.success) {
				...
				// フィールド初期化
				MstCommon.createField(md0011.partsInfo, res.entity, displayMode);
				...
				pager.search(cond).done(function(accRes) {
					...
					// フィールド初期化
					MstCommon.createField(md0011.gridPartsInfo, accRes.accList, displayMode);
	...
	partsInfo : [
		...
		{'selectorKey':'#lndNm', 'id':'lndNm', 'class':'form-control', 'data-role':'text', 'data-field':'lndNm', 'data-validate':'{"maxlength":3,"pattern":"alpha"}', 'readonly':false, 'disabled':false, 'listName':'', 'rows':'','checkLabel':'','style':''},
