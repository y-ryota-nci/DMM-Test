$(function() {
	...
	var seqNoAuthTransfer = NCI.getQueryString("seqNoAuthTransfer");
	var userCode = NCI.getQueryString("userCode");
	var params = {
	...
	NCI.init("/ws0111/init", params).done(function(res) {
		if (res && res.success) {
			...
			if (res.target) {
				...
				$('#corporationCode').val(corporationCode);
	...
	$(document)
		...
		.on('click', '#btnRegist', function(ev) {
			...
			if (NCI.confirm(msg, function() {
				...
				NCI.post("/ws0111/regist", params).done(function(res) {
					if (res && res.success) {
		...
		.on('click', '#btnUpdate', function(ev) {
			...
			if (NCI.confirm(msg, function() {
				var entry = $('#corporationCode').val()
					...
					+ "\t" + $('#validStartDate').val()
					+ "\t" + $('#validEndDate').val();
				NCI.post("/ws0111/update", {entry : entry}).done(function(res) {
					if (res && res.succes) {
					}
				});
			}));
		...
		.on('click', '#btnDelete', function(ev) {
			...
			if (NCI.confirm(msg, function() {
				...
				NCI.post("/ws0111/delete", {entry : entry}).done(function(res) {
					if (res && res.success) {
