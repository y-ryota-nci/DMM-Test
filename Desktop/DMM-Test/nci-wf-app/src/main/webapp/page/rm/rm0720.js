$(function() {
	var pager = new Pager($('#seach-result'), '/rm0720/search', search).init();
	...
	NCI.init("/rm0720/init", params).done(function(res) {
		if (res && res.success) {
	...
	$(document)
		...
		.on('click', '#btnUpdate', function(ev) {
			...
			if (NCI.confirm(msg, function() {
				...
				NCI.post("/rm0720/update", params).done(function(res) {
		...
		.on('click', '#btnDelete', function(ev) {
			...
			if (NCI.confirm(msg, function() {
				...
				NCI.post("/rm0720/delete", params).done(function(res) {
					search(1);
				});
			}));
		})
...
function openEntry(corporationCode, menuRoleCode, seqNoMenuRoleDetail) {
	NCI.redirect("./rm0730.html?corporationCode=" + corporationCode + "&menuRoleCode=" + menuRoleCode + "&seqNoMenuRoleDetail=" + seqNoMenuRoleDetail);
}
