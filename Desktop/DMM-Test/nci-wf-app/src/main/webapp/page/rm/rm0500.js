$(function() {
	var pager = new Pager($('#seach-result'), '/rm0500/search', search).init();

	var params = {
		messageCds : []
	};
	NCI.init("/rm0500/init", params).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
	...
	function openEntry(corporationCode, menuRoleCode) {
		NCI.redirect("./rm0510.html?corporationCode=" + corporationCode + "&menuRoleCode=" + menuRoleCode);
	}
});
