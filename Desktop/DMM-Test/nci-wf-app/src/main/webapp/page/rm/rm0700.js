$(function() {
	var pager = new Pager($('#seach-result'), '/rm0700/search', search).init();

	var params = {
			messageCds : ['MSG0003', 'targetRow']
	};
	NCI.init("/rm0700/init", params).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
	...
	.on('click', '#btnAdd', function(ev) {
		NCI.redirect("./rm0710.html?corporationCode=" + NCI.loginInfo.corporationCode);
	})
	// ダウンロードボタン
	...
	function downloadMenuRole() {
		...
		NCI.download('/rm0700/downloadZip', params);
	}
