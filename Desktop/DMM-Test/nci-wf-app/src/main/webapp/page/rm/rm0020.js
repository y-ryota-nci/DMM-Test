$(function() {
	var pager = new Pager($('#seach-result'), '/rm0020/search', search).init();
	...
	NCI.init("/rm0020/init", params).done(function(res) {
		if (res && res.success) {
	...
	$(document)
		...
		.on('click', '#btnUpdate', function(ev) {
			...
			if (NCI.confirm(msg, function() {
				...
				NCI.post("/rm0020/update", params).done(function(res) {
		...
		.on('click', '#btnDelete', function(ev) {
			...
			if (NCI.confirm(msg, function() {
				var params = { deleteMenuRoleDetails : [] };
				...
				NCI.post("/rm0020/delete", params).done(function(res) {
					search(1);
				});
			}));
		})
...
function openEntry(corporationCode, menuRoleCode, seqNoMenuRoleDetail) {
	NCI.redirect("./rm0030.html?corporationCode=" + corporationCode + "&menuRoleCode=" + menuRoleCode + "&seqNoMenuRoleDetail=" + seqNoMenuRoleDetail);
}
