$(function() {
	...
	NCI.init("/rm0903/init", params).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
		...
		$(document)
			...
			.on('click', '#btnBack', function(ev) {
				NCI.redirect("./rm0902.html?corporationCode=" + $("#corporationCode").val() + "&menuRoleCode=" + $("#menuRoleCode").val());
			})
			// 登録ボタン
			.on('click', '#btnRegister', function(ev) {
				...
				if (NCI.confirm(msg, function() {
					...
					NCI.post("/rm0903/insert", params2).done(function(res) {
			...
			.on('click', '#btnUpdate', function(ev) {
				...
				if (NCI.confirm(msg, function() {
					...
					NCI.post("/rm0903/update", params2).done(function(res) {
						if (res && res.success && res.menuRoleDetail) {
							if (res.menuRoleDetail.deleteFlag == '1') {
								$('#btnUpdate').prop('disabled', false);
								$("#validStartDate").prop('disabled', true);
			...
			.on('click', '#btnDelete', function(ev) {
				...
				if (NCI.confirm(msg, function() {
					...
					NCI.post("/rm0903/delete", params2).done(function(res) {
						if (res && res.success && res.menuRoleDetail) {
						}
					});
				}));
