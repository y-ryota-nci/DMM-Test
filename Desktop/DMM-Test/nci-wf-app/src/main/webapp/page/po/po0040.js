$(function() {
	NCI.init('/po0040/init', { "messageCds" : [] }).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
			...
			FileUploader.setup("div.dragZone", "/po0040/upload", false, complete, prepareCallback);
			// ファイルコントロールによるファイルアップロード
			FileUploader.setup("input[type=file]", "/po0040/upload", false, complete, prepareCallback);
		}
	});
