$(function() {
	const pager = new Pager($('#seach-result'), '/dc0210/search', search).init();
	pager.responsiveTable.modifyTR = modifyTR;	// 標準機能を上書き

	const params = { messageCds : ['MSG0120', 'trayConfig'] };
	NCI.init("/dc0210/init", params).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
			...
			.on('click', '#btnDelete', function(ev) {
				...
				if (NCI.confirm(msg, function() {
					...
					NCI.post("/dc0210/delete", params).done(function(res) {
						search(1, true);
					});
				}));
			})
...
//				NCI.download('/dc0210/downloadZip/' + NCI.loginInfo.corporationCode);
//			}).prop('disabled', false);
		}
	});
	...
	function createCondition(pageNo) {
		...
		// デフォルトソート条件
		if (!cond.sortColumn) {
			cond.sortColumn = 'TC.SORT_ORDER, TC.DOC_TRAY_CONFIG_ID';
			cond.sortAsc = true;
		}
		return cond;
	}
	...
	function openEntry(trayConfigId, version) {
		let url = "./dc0220.html?from=dc0210";
		if (trayConfigId) {
			url += "&docTrayConfigId=" + trayConfigId + "&version=" + version;
		}
		NCI.redirect(url);
	}
