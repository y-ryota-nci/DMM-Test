$(function() {
	const pager = new Pager($('#seach-result'), '/dc0240/search', search).init();
	pager.responsiveTable.modifyTR = modifyTR;	// 標準機能を上書き

	const params = { messageCds : ['MSG0120', 'trayConfig'] };
	NCI.init("/dc0240/init", params).done(function(res) {
		if (res && res.success) {
			// カレンダー（年月日）
			...
			.on('click', '#btnDelete', function(ev) {
				...
				if (NCI.confirm(msg, function() {
					...
					NCI.post("/dc0240/delete", params).done(function(res) {
						search(1, true);
					});
				}));
			})
...
//				NCI.download('/dc0240/downloadZip/' + NCI.loginInfo.corporationCode);
//			}).prop('disabled', false);
		}
	});
	...
	function openEntry(trayConfigId, version) {
		let url = "./dc0220.html?from=dc0240";
		if (trayConfigId) {
